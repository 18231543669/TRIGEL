...
class GraphCacheManager:
    ...
    def _generate_key(self, data: np.ndarray, k: int, method: str = "knn_binary") -> str:
        ...
        key_str = f"graph_{method}_{shape_str}_k{k}_{stats_str}"
        return hashlib.md5(key_str.encode()).hexdigest()
    
    def get(self, data: np.ndarray, k: int, method: str = "knn_binary") -> Optional[object]:
        """从缓存获取KNN图"""
        cache_key = self._generate_key(data, k, method)
    ...
    def set(self, data: np.ndarray, k: int, graph_data: object, method: str = "knn_binary") -> str:
        """保存KNN图到缓存"""
        cache_key = self._generate_key(data, k, method)

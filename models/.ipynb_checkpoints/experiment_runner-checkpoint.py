...
class ExperimentRunner:
    ...
    def _build_graph_structures(self, data_info: Dict) -> Optional[List[torch.Tensor]]:
        ...
        # 如果是超参数搜索模式，定期打印缓存统计
        if self.is_hyperparameter_search and len(edge_indices) == num_omics:
            try:
                from utils.graph_cache import GraphCacheManager
                stats = GraphCacheManager().stats()
                if stats['total_queries'] % 20 == 0:
                    print(f"  缓存状态: {stats['cache_size']}图, 命中率: {stats['hit_rate']:.1%}")
            except ImportError:
                pass
    ...
    def _initialize_model(self, data_info: Dict, data_splits: Dict) -> tuple:
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=self.config['lr'], weight_decay=5e-3)

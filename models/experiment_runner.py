"""
实验运行器模块（集成错误分析 + 性能优化版 + 新融合模块支持）
负责协调和执行完整的实验流程，并记录错误样本用于后续分析

改进点：
1. 支持新的融合模块超参数 (fusion_num_heads, fusion_dropout)
2. 支持新的类别分层超参数
3. 数据加载缓存：第1次运行加载，后续运行复用
4. 图结构缓存：第1次运行构建，后续运行复用
"""
...
class ExperimentRunner:
    ...
    def run_single_experiment(self, run_id: int = 1, 
        ...
        try:
            ...
            # 超参数搜索模式下剥离大对象，减少长时间搜索内存压力
            if self.is_hyperparameter_search and isinstance(final_results, dict):
                for bulky_key in (
                    'history',
                    'training_history',
                    'config',
                    ...
                    final_results.pop(bulky_key, None)
    ...
    def _build_graph_structures(self, data_info: Dict) -> Optional[List[torch.Tensor]]:
        """构建图结构：支持读取预计算adj，或按cosine_k现场KNN构图。"""
        edge_indices = []
        num_omics = data_info['num_omics']
        graph_source = self.config.get('graph_source', 'precomputed')
        adj_dir = self.config.get('adj_dir') or self.config['data_dir']
        cosine_k = int(self.config.get('cosine_k', 5))

        for i in range(num_omics):
            try:
                if graph_source == 'knn':
                    if not self.is_hyperparameter_search:
                        print(f"\n构建组学 {i + 1}/{num_omics} 的KNN图结构 (k={cosine_k})")
                    omics_data = np.vstack([data_info['train_data'][i], data_info['test_data'][i]])
                    edge_index = construct_adjacency(omics_data, k=cosine_k, use_cache=True)
                else:
                    if not self.is_hyperparameter_search:
                        print(f"\n加载组学 {i + 1}/{num_omics} 的adj图结构")
                    adj_path = os.path.join(adj_dir, f"adj{i + 1}.csv")
                    edge_index = load_precomputed_adjacency_csv(adj_path)

                edge_indices.append(edge_index)

                if not self.is_hyperparameter_search:
                    if graph_source == 'knn':
                        print(f"图 {i + 1} 有 {edge_index.shape[1]} 条边 (from knn, k={cosine_k})")
                    else:
                        print(
                            f"图 {i + 1} 有 {edge_index.shape[1]} 条边 "
                            f"(from adj{i + 1}.csv, adj_dir={adj_dir})"
                        )
            except Exception as e:
                print(f"构建图结构失败: {e}")
                return None
        ...
        if self.config.get('use_augmentation', False):
            ...
            try:
                ...
                default_params = {
                    ...
                    # ✨ 新增：类别分层参数
                    'class_tier_severe_threshold': self.config.get('class_tier_severe_threshold', 0.05),
                    'class_tier_moderate_threshold': self.config.get('class_tier_moderate_threshold', 0.15),
                    'class_tier_intermediate_threshold': self.config.get('class_tier_intermediate_threshold', 0.30),
                    'effective_num_beta': self.config.get('effective_num_beta', 0.9999),
                }
    ...
    def _initialize_model(self, data_info: Dict, data_splits: Dict) -> tuple:
        """初始化模型和优化器（✨ 支持新的融合模块参数）"""
        
        # ✨ 新增：获取融合模块参数
        fusion_num_heads = self.config.get('fusion_num_heads', 4)
        fusion_dropout = self.config.get('fusion_dropout', 0.3)
        
        if not self.is_hyperparameter_search:
            print(f"\n融合模块配置: num_heads={fusion_num_heads}, dropout={fusion_dropout}")
    ...
    def _train_model(self, model, optimizer, criterion, augmented_data: Dict,
        ...
        cl_warmup_epochs = int(self.config.get('cl_warmup_epochs', 0))
        total_weight = float(cb_weight + cl_weight)
        ...
        for epoch in progress_bar:
            ...
            if self.config.get('use_contrastive', False) and isinstance(model, GCNContrastiveModel):
                ...
                # 对比学习warmup：前期降低对比损失干扰，提升小样本稳定性
                if cl_warmup_epochs > 0:
                    warmup_ratio = min(1.0, float(epoch + 1) / float(cl_warmup_epochs))
                    current_cl_weight = cl_weight * warmup_ratio
                    current_cb_weight = max(total_weight - current_cl_weight, 0.0)
                else:
                    current_cl_weight = cl_weight
                    current_cb_weight = cb_weight
                ...
                contrastive_loss = model.compute_contrastive_loss(
                    ...
                    labels=data_splits['y_tensor'],
                    train_mask=train_mask
                )
                total_loss = (
                    current_cb_weight * classification_loss +
                    current_cl_weight * contrastive_loss
                )
                ...
                history.setdefault('current_cb_weight', []).append(float(current_cb_weight))
                history.setdefault('current_cl_weight', []).append(float(current_cl_weight))
            else:
    ...
    def _finalize_experiments(self, all_results: List[Dict], main_result_dir: str) -> None:
        ...
        top_k = int(self.config.get('experiment_top_k', 0))
        avg_results = calculate_average_results(all_results, top_k=top_k)
        if avg_results:

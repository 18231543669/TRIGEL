...
class GCNContrastiveModel(nn.Module):
    ...
    def _encode_features(
        ...
        """Encode nodes into the shared multi-omics fusion feature space."""
        omics_features = []
        ...
        fusion_feature = self.fusion_attention(omics_features)
        return torch.cat(omics_features + [fusion_feature], dim=1)
    ...
    def forward(self, x: torch.Tensor, edge_indices: List[torch.Tensor],
        ...
        Returns:
            logits: 分类logits [num_nodes, num_classes]
        """
        final_features = self._encode_features(x, edge_indices, edge_weights)
        logits = self.classifier(final_features)
    ...
    def get_classifier_embeddings(
        ...
        """Return supervised penultimate features used by the classifier."""
        final_features = self._encode_features(x, edge_indices, edge_weights)
        return self.classifier[1](self.classifier[0](final_features))
